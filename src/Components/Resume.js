...
class Resume extends Component {
  render() {
    ...
    if(this.props.data){
      ...
      var verv = this.props.data.verv.map(function(verv){
        return <div key={verv.company}><h3>{verv.title}</h3>
            <p className="info">{verv.company}<span>&bull;</span> <em className="date">{verv.years}</em></p>
            <p>{verv.description}</p>
        </div>
      })
